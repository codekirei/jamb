 ...
 */
const makeCanonical = (host, slug) => slug === 'index'
  ? host
  : `${host}/${slug}/`
...
const addCanonical = host => ob =>
  Object.assign(ob, {canonical: makeCanonical(host, ob.url)})

/**
  Make path to write to.

  @param {String} dist - root dist dir
  @param {String} slug - slug for page
  @returns {String} local path
 */
const makePath = (dist, slug) => slug === 'index'
  ? p.join(dist, 'index.html')
  : p.join(dist, slug, 'index.html')
...
/**
  ...
  @returns {Function} curried function that mutates data object
 */
const addPath = dist => ob => Object.assign(ob, {path: makePath(dist, ob.url)})
